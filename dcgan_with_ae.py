...
#DCGAN: input image -> 4 conv -> output prob
...
model_file_path = "./"
model_file_name = os.path.join(model_file_path ,"dcgan_with_ae.ckpt")
img_path = "./img/"
...
dcgan_h_conv1_img = tf.nn.elu(tf.nn.bias_add(conv2d(input_x,dcgan_W_conv1,[1,2,2,1]),dcgan_b_conv1))

dcgan_W_conv2 = weight_variable([4,4,DIM7,DIM8])
dcgan_b_conv2 = bias_variable([DIM8])
dcgan_h_conv2_img = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv1_img,dcgan_W_conv2,[1,2,2,1]),dcgan_b_conv2))

dcgan_W_conv3 = weight_variable([4,4,DIM8,DIM9])
dcgan_b_conv3 = bias_variable([DIM9])
dcgan_h_conv3_img = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv2_img,dcgan_W_conv3,[1,2,2,1]),dcgan_b_conv3))

dcgan_W_conv4 = weight_variable([4,4,DIM9,DIM10])
dcgan_b_conv4 = bias_variable([DIM10])
dcgan_h_conv4_img = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv3_img,dcgan_W_conv4,[1,2,2,1]),dcgan_b_conv4))


#convolution(DCGAN generator)
dcgan_h_conv1_gen = tf.nn.elu(tf.nn.bias_add(conv2d(gen_h_deconv5,dcgan_W_conv1,[1,2,2,1]),dcgan_b_conv1))
dcgan_h_conv2_gen = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv1_gen,dcgan_W_conv2,[1,2,2,1]),dcgan_b_conv2))
dcgan_h_conv3_gen = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv2_gen,dcgan_W_conv3,[1,2,2,1]),dcgan_b_conv3))
dcgan_h_conv4_gen = tf.nn.elu(tf.nn.bias_add(conv2d(dcgan_h_conv3_gen,dcgan_W_conv4,[1,2,2,1]),dcgan_b_conv4))
...
dcgan_W_id_list = [dcgan_W_id4]
dcgan_b_id_list = [dcgan_b_id4]
...
io.imsave("img_z_AE.jpg",(all_img+1.0)/2.0)
"""
sess.close()
